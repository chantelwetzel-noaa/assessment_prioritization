...
summarize_frequency <- function(
  ...
  fishery_importance <- data.frame(
    ...
    Total = commercial$Factor_Score +
      tribal$Factor_Score +
      recreational$Factor_Score
  )

  fishery_importance <- fishery_importance[
    order(fishery_importance[, "Total"], decreasing = TRUE),
  ]
  fishery_importance[, "Rank"] <- 1:nrow(fishery_importance)
  fishery_importance <- with(
    fishery_importance,
    fishery_importance[order(fishery_importance[, "Species"]), ]
  )
  ...
  for (sp in 1:nrow(df)) {
    ...
    if (!is.na(df$Recruitment_Variation[sp])) {
      df$Recruitment_Adjustment[sp] <-
        ifelse(
          df$Recruitment_Variation[sp] >= 1,
          -0.2,
          ifelse(
            df$Recruitment_Variation[sp] < 1 &
              df$Recruitment_Variation[sp] >= 0.35,
            0,
    ...
    df$Importance_Adjustment[sp] <-
      ifelse(
        fishery_importance$Rank[sp] <= change,
        -0.2,
        ifelse(
          fishery_importance$Rank[sp] > change &
            fishery_importance$Rank[sp] < 2 * change,
          0,
          0.2
        )
      )
    ...
    df$Ecosystem_Adjustment[sp] <-
      ifelse(
        ecosystem$Rank[sp] <= change,
        -0.2,
        ifelse(
          ecosystem$Rank[sp] > change & ecosystem$Rank[sp] < 2 * change,
          0,
          0.2
        )
      )
    ...
    df$Adjusted_Maximum_Age[sp] <- ifelse(
      df$Total_Adjustment[sp] == 0,
      df$Mean_Maximum_Age[sp],
  ...
  for (sp in 1:nrow(df)) {
    df$Target_Assessment_Frequency[sp] <- ifelse(
      df$Adjusted_Maximum_Age[sp] <= age_metric[2],
      4,
      ifelse(
        df$Adjusted_Maximum_Age[sp] > age_metric[2] &
          df$Adjusted_Maximum_Age[sp] < age_metric[3],
        6,
        ifelse(
          df$Adjusted_Maximum_Age[sp] >= age_metric[3] &
            df$Adjusted_Maximum_Age[sp] < age_metric[4],
          8,
          10
        )
      )
    ...
    df$Years_Past_Target_Frequency[sp] <- ifelse(
      is.na(df$Last_Assessment_Year[sp]),
      4,
      ifelse(
        df$Years_Since_Assessment[sp] - df$Target_Assessment_Frequency[sp] > 0,
        df$Years_Since_Assessment[sp] - df$Target_Assessment_Frequency[sp],
        0
      )
    )

    # If we are at or more than 10 years since last assessment add +1
    df$Ten_Years_or_Greater[sp] <- ifelse(
      df$Years_Since_Assessment[sp] >= 10,
      3,
      0
    )
    ...
    df$Less_Than_6_Years_Update[sp] <-
      ifelse(
        df$Years_Since_Assessment[sp] <= 6 & abundance$SSC_Rec[sp] == "Update",
        1,
        0
      )
